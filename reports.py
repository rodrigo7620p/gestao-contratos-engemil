...
def _add_bullet_list(doc, items, *, numbered=False):
    for index, item in enumerate(items, start=1):
        ...
        _set_run_font(paragraph.add_run(prefix), bold=True, color=BURGUNDY)
        _set_run_font(paragraph.add_run(str(item)))


def _add_titled_items(doc, items):
    """Lista de itens {titulo, texto} — usado para riscos e encaminhamentos."""
    for item in items or []:
        titulo = (item or {}).get("titulo")
        texto = (item or {}).get("texto")
        if not _meaningful(texto) and not _meaningful(titulo):
            continue
        paragraph = doc.add_paragraph()
        ...
        paragraph.paragraph_format.line_spacing = 1.25
        paragraph.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
        if _meaningful(titulo):
            _set_run_font(paragraph.add_run(f"{titulo}. "), bold=True, color=BURGUNDY)
        if _meaningful(texto):
            _set_run_font(paragraph.add_run(str(texto)))


def generate_bid_viability_docx(report: dict, bid_process: dict) -> bytes:
    """Monta a Nota Técnica de Análise de Viabilidade (documento gerado por
    IA a partir dos editais do PNCP) no mesmo padrão visual dos demais
    documentos ENGEMIL — logo, cores institucionais, tabelas no mesmo
    estilo da Ficha Contratual."""
    doc = _new_document()
    agency = bid_process.get("agency") or "órgão não informado"
    process_ref = bid_process.get("edital_number") or bid_process.get("process_number") or "s/nº"
    _add_title(
        doc,
        "NOTA TÉCNICA DE ANÁLISE DE VIABILIDADE",
        f"{process_ref} · {agency}",
    )

    _add_definition_table(doc, [
        ("Objeto", report.get("objeto")),
        ("Valor estimado", report.get("valor_estimado_texto")),
        ("Proposta ENGEMIL", report.get("proposta_engemil_texto")),
        ("Situação", report.get("situacao_texto")),
        ("Destinatário", "Diretoria ENGEMIL"),
        ("Data", short_date(datetime.now())),
    ])

    _add_heading(doc, "1. Conclusão executiva")
    for paragraph_text in str(report.get("conclusao_executiva") or "").split("\n\n"):
        _add_text(doc, paragraph_text.strip(), justified=True)
    recomendacao = report.get("recomendacao")
    if _meaningful(recomendacao):
        _add_text(doc, f"Recomendação: {recomendacao}", justified=True, bold=True)

    discount = report.get("analise_desconto") or {}
    if _meaningful(discount.get("explicacao")) or discount.get("linhas_tabela"):
        _add_heading(doc, "2. Análise do desconto real")
        _add_text(doc, discount.get("explicacao"), justified=True)
        columns = discount.get("colunas_tabela") or []
        rows = discount.get("linhas_tabela") or []
        if columns and rows:
            _add_records_table(doc, columns, rows, [1] * len(columns))
        limites = discount.get("limites_controle") or []
        if limites:
            _add_text(doc, "Limites de controle do edital:", bold=True)
            _add_bullet_list(doc, limites)

    breakeven = report.get("ponto_equilibrio") or {}
    if _meaningful(breakeven.get("explicacao")) or breakeven.get("linhas_tabela"):
        _add_heading(doc, "3. Ponto de equilíbrio por cenário")
        _add_text(doc, breakeven.get("explicacao"), justified=True)
        columns = breakeven.get("colunas_tabela") or []
        rows = breakeven.get("linhas_tabela") or []
        if columns and rows:
            _add_records_table(doc, columns, rows, [1] * len(columns))

    pros = report.get("pontos_favoraveis") or []
    if pros:
        _add_heading(doc, "4. Pontos favoráveis")
        _add_bullet_list(doc, pros)

    riscos = report.get("riscos") or []
    if riscos:
        _add_heading(doc, "5. Contras e riscos")
        _add_titled_items(doc, riscos)

    encaminhamentos = report.get("encaminhamentos") or []
    if encaminhamentos:
        _add_heading(doc, "6. Encaminhamentos propostos")
        _add_titled_items(doc, encaminhamentos)

    sintese = report.get("sintese_final")
    if _meaningful(sintese):
        _add_heading(doc, "Síntese")
        _add_text(doc, sintese, justified=True, bold=True)

    _add_text(
        doc,
        "Documento gerado automaticamente por análise de inteligência artificial "
        "a partir dos documentos publicados no PNCP — revise antes de basear "
        "qualquer decisão definitiva exclusivamente nele.",
        color=MID_GRAY,
    )

    return _save_bytes(doc, "NOTA TÉCNICA DE VIABILIDADE")
